...
from model import CulinaryAssistant
...
async def search_agent(prompt, name_agent: str, request: Request):
    ...
    print(ip)
    print(port)
    chief_agent = CulinaryAssistant("Hanson family Chief")
    flag_access_owner_user = is_agent_exist(ip,name_agent, "Owner_properties.csv")
    if not (flag_access_owner_user):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=f"Your are not allowed to access {chief_agent.name}.")

    
    cook_res = chief_agent.cook(prompt)
    return cook_res
